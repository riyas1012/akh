 ...
 */				if (response[index].reporting_name == 'OVL'){
				...
				if (response[index].item_order_status == 0)
					...
					active = "<label class='badge badge-primary'><i class='fa fa-times'></i> </label>";
				else if (response[index].item_order_status == 4)
						...
						success:function(response) {
							...
							if(response.success == true) {
								...
								$.ajax({
									...
									type: "post",
									dataType: "json",
									data: {'id':newOrderId, action: 'Category Managerxx'},
									dataType: 'json',
									success:function(response) {
...
/* 			 for (var x = 0; x < comment.length; x++) {       						
		...
		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax': { url: 'php_action/fetchOrder.php'},
 			 'columns': [
				{
					null:null
				},
...
/* 			contentType: false,
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
		   ...
		   for (var x = 0; x < itemSubat.length; x++) {       			
			...
			var sideOpeing = document.getElementsByName('ovl_tent_side_opening[]');				
			var validateItemSubcat;
			for (var x = 0; x < sideOpeing.length; x++) {       			
					var sideOpeingId = sideOpeing[x].id;
					console.log('sideOpeingId: '+sideOpeingId);
					console.log('sideOpeing Value: '+sideOpeing[x].value);
				 if(sideOpeing[x].value == ''){	    		    	
					 $("#"+sideOpeingId+"").after('<p class="text-danger"> This Field is required </p>');
					 $("#"+sideOpeingId+"").closest('.form-group').addClass('has-error');	    		    	    	
				 } else {      	
					 $("#"+sideOpeingId+"").closest('.form-group').addClass('has-success');	    		    		    	
				 }          
			 } // for
	...
	} else if(divRequest == 'amendOrd') {
		//$("#orderDate").datepicker();
		console.log('inside amendOrd');
		// edit order form function
		console.log($('#orderId').val());

		$("#amendOrderForm").unbind('submit').bind('submit', function() {

			//$("#amendOrderBtn").click('submit', function() {
			// alert('amendOrderForm');
			var form = $(this);
					...
					$.ajax({
						...
						success:function(response) {
							...
							if(response.success == true) {
								...
								$(".mainAmendCard").addClass('div-hide');

								orderItemId = response.orderItemId;
								console.log('orderItemId'+orderItemId);
								// send emails -- category managers dont need to get notified. added xx to action name
 								/* $.ajax({
									url: "php_action/sendEmail.php",
									type: "post",
									dataType: "json",
									async: false,
									data: {'id':orderItemId, action: 'Planner Amendedxx'},
									dataType: 'json',
									success:function(response) {
										console.log(response.emailSuccess);
										// reset button
...
function amendOrder(orderItemId) {
	if(orderItemId) {
		//alert(1);
		$("#amendOrderBtn").unbind('click').bind('click', function() {
			//alert(2);
			$("#amendOrderBtn").button('loading');
			amendReason = $('#amend_reason').val();
			$.ajax({
				url: 'php_action/amendOrder.php',
				type: 'post',
				data: {orderItemId : orderItemId, amendReason: amendReason},
				dataType: 'json',
				success:function(response) {
					//alert(3)
					$("#amendOrderBtn").button('reset');
					...
					if(response.success == true) {
						...
						// remove the mesages
	          			$(".alert-success").delay(500).show(10, function() {
							...
							$.ajax({
								...
								type: "post",
								dataType: "json",
								data: {'id':orderItemId, action: 'Amendment'},
								dataType: 'json',
								async: false,
								success:function(response) {
									console.log(response.emailSuccess);
						...
						// error messages
						$(".amendOrderMessages").html('<div class="alert alert-warning">'+
	            			'<button type="button" class="close" data-dismiss="alert">&times;</button>'+
	            			'<strong><i class="fa fa-times"></i></strong> '+ response.messages +
	          				'</div>');
