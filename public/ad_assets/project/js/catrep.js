...
$(document).ready(function() {
	...
	manageTable = $('#manageTable').DataTable({
		ajax: 'php_action/fetchCatrep.php',
        order: []
    });
	...
	$("#addModalBtn").unbind('click').bind('click', function() {
		...
		$("#submitForm").unbind('submit').bind('submit', function() {
			...
			if(!addStatus) {
				$("#addStatus").after('<p class="text-danger"> Status field is required</p>');
				//$('#OVLStatus').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#addStatus").find('.text-danger').remove();
				// success out for form 
...
/* 					url : 'php_action/create.php',
		          ...
		          '</div>');
							...
							// remove the mesages
		          $("#add-messages").delay(500).show(10, function() {
            ...
            $.ajax({
              ...
              // 		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
                    ...
                    $("#editForm").unbind('submit').bind('submit', function() {
                        ...
                        if(editName == "") {
                            $("#editName").after('<p class="text-danger">OVL Name field is required</p>');
                        ...
                        if(!editStatus) {
                            $("#editStatus").after('<p class="text-danger">OVL Status field is required</p>');
    ...
    function removeRecord(id) {
        console.log("inside: removeOVL("+id+")");
        if(id) {
            ...
            $("#removeBtn").unbind('click').bind('click', function() {
                ...
                $.ajax({
                    url: 'php_action/removeCatrep.php',
                    type: 'post',
                    data: {id:id},
                    dataType: 'json',
                    success:function(response) {
