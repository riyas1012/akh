 ...
 */
    // add event modal btn clicked
    		// select event form
		$("#selectEventForm").unbind('submit').bind('submit', function() {
            console.log("inside selectEventForm");
            //alert(1);
            var selectedEventId = $('#changeName').val();
            var action = 'changeEvent';
            console.log('selectedEventId: '+selectedEventId);
            console.log('action: '+action);
			$.ajax({
                    url : 'php_action/fetchSelectedEvent.php',
                    type: 'post',
                    data: {eventId:selectedEventId, action:action},
					dataType: 'json',
                    cache: false,
                    //async: false,
					//contentType: false,
					...
					//	if(response.success == true) {
							...
							$('#change-event-messages').html('<div class="alert alert-success">'+
